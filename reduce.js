...
const getlongestString = function (stringOne, stringTwo) {
  return stringTwo.length > stringOne.length ? stringTwo : stringOne;
};

const getShortestString = (stringOne, stringTwo) => {
  return stringTwo.length < stringOne.length ? stringTwo : stringOne;
};
...
const minOf = function (numbers) {
  return numbers.reduce(function (numOne, numTwo) {
...
const findSumOfEvenSquares = function (numbers) {
  ...
  const squareOfevens = evens.map((number) => number ** 2);
  return squareOfevens.reduce(add, 0);
};
...
const longestWord = function (words) {
  return words.reduce(getlongestString, "");
};
...
const groupByDate = function (records) {
  return records.reduce(function (groupedData, { date, value }) {
    if (!groupedData[date]) {
      groupedData[date] = [];
    }

    groupedData[date] = [...groupedData[date], value];
    return groupedData;
  }, {});
};
...
const sumByCategory = function (items) {
  return items.reduce(function (sum, { category, value }) {
    if (!sum[category]) {
      sum[category] = 0;
    }

    sum[category] = value + sum[category];
    return sum;
  }, {});
};
